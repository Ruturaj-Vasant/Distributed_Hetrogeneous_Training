...
def training_loop(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        accum_samples = 0
        accum_count   = 0
        model.zero_grad()   # zero once; only re-zero after each sync
        ...
        for batch_idx, (images, labels) in enumerate(loader):
            ...
            # Divide by grad_accum so the accumulated gradient is the average
            # of all N batches, not the sum.
            ...
            if accum_count < grad_accum and not is_last_batch:
                # Still accumulating — don't sync yet
                if batch_idx % 20 == 0:
                    log.info(
            ...
            # ── Apply weight delta, reset accumulators ────────────────────
